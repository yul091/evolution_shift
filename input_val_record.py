...
class InputVal(object):
    ...
    # DATASETS = ['shift2', 'val', 'shift1']
    DATASETS = ['shift1', 'val', 'shift2']
    RES_FILE = 'input_val'
    ...
    def get_acc(self, threshold, uncertainty_score, data_name, uncertainty_name):
        ...
        if len(remained_index) == len(uncertainty_score):
            ...
            return acc, improve_level, coverage
    ...
    def search_coverage(self, data_name, uncertainty, uncertainty_name, txt_file):
        ...
        record = []
        for threshold in torch.arange(0, 1, self.gap).flip(0):
            threshold = round(threshold.item(), 4)
            if uncertainty_name not in ['mutation', 'dissactor']:
                acc, improve_level, coverage = self.get_acc(
                    threshold, uncertainty[data_name], data_name, uncertainty_name
                )
            else:
                acc, improve_level, coverage = self.get_acc(
                    threshold, uncertainty[data_name][0], data_name, uncertainty_name
                )
            record.append([acc, improve_level, coverage])
